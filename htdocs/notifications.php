...
$activity=$pdo->prepare("SELECT l.*,a.username actor_name FROM user_activity_log l
...
<?php if($pending): ?><div class="pending"><?php if(!empty($pending['division'])):?><a class="badge" href="admin.php?tab=transfers"><?php echo h(t('notifications_division_pending')); ?>: <?php echo $pending['division'];?></a><?php endif;?><?php if(!empty($pending['appeals'])):?><a class="badge" href="admin.php?tab=moderation"><?php echo h(t('notifications_appeals_pending')); ?>: <?php echo $pending['appeals'];?></a><?php endif;?></div><?php endif;?>
