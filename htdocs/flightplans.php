...
<section class="card"><h2><?php echo h(t('flightplan_history')); ?></h2><div class="scroll"><table><thead><tr><th><?php echo h(t('admin_time')); ?></th><th><?php echo h(t('flightplan_callsign')); ?></th><th><?php echo h(t('flightplan_route')); ?></th><th><?php echo h(t('flightplan_status')); ?></th><th></th></tr></thead><tbody><?php foreach($plans as $p):?><tr><td><?php echo h($p['updated_at']);?></td><td><?php echo h($p['callsign']);?></td><td><a href="airport.php?icao=<?php echo rawurlencode($p['departure_airport']); ?>"><?php echo h($p['departure_airport']); ?></a> → <a href="airport.php?icao=<?php echo rawurlencode($p['arrival_airport']); ?>"><?php echo h($p['arrival_airport']); ?></a></td><td><?php echo h(t('flightplan_status_'.$p['status']));?></td><td><a class="button" href="?edit=<?php echo (int)$p['id'];?>"><?php echo h(t('flightplan_edit'));?></a></td></tr><?php endforeach;?></tbody></table></div><nav class="pager"><?php for($i=1;$i<=$pages;$i++):?><a href="?page=<?php echo $i;?>"><?php echo $i;?></a><?php endfor;?></nav></section>
