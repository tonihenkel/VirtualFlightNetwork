...
try {
    ...
    if($runway===''&&$landingRunway===''&&$gate===''&&$sid===''&&$direct===''&&$star===''&&$altitude===''){
        $delete=$pdo->prepare("DELETE FROM atc_aircraft_clearances WHERE pilot_session_token=:token");
        $delete->execute(['token'=>(string)$pilot['session_token']]);
        clearanceReply(true,['message'=>'clearance_deleted','clearance'=>[
            'departure_runway'=>'','landing_runway'=>'','gate'=>'','sid'=>'','direct'=>'','star'=>'','altitude'=>''
        ]]);
    }
    $legacyType=$sid!==''?'SID':($direct!==''?'DIRECT':($star!==''?'STAR':'DIRECT'));
    $legacyValue=$sid!==''?$sid:($direct!==''?$direct:$star);
    $save=$pdo->prepare("INSERT INTO atc_aircraft_clearances (pilot_session_token,pilot_callsign,clearance_type,clearance_value,cleared_departure_runway,cleared_landing_runway,cleared_gate,cleared_sid,cleared_direct,cleared_star,cleared_altitude,issued_by_user_id,issued_by_callsign) VALUES (:token,:callsign,:type,:value,:runway,:landing_runway,:gate,:sid,:direct,:star,:altitude,:user_id,:atc_callsign) ON DUPLICATE KEY UPDATE pilot_callsign=VALUES(pilot_callsign),clearance_type=VALUES(clearance_type),clearance_value=VALUES(clearance_value),cleared_departure_runway=VALUES(cleared_departure_runway),cleared_landing_runway=VALUES(cleared_landing_runway),cleared_gate=VALUES(cleared_gate),cleared_sid=VALUES(cleared_sid),cleared_direct=VALUES(cleared_direct),cleared_star=VALUES(cleared_star),cleared_altitude=VALUES(cleared_altitude),issued_by_user_id=VALUES(issued_by_user_id),issued_by_callsign=VALUES(issued_by_callsign),updated_at=NOW()");
    $save->execute(['token'=>(string)$pilot['session_token'],'callsign'=>$callsign,'type'=>$legacyType,'value'=>$legacyValue,'runway'=>$runway,'landing_runway'=>$landingRunway,'gate'=>$gate,'sid'=>$sid,'direct'=>$direct,'star'=>$star,'altitude'=>$altitude,'user_id'=>(int)$atc['user_id'],'atc_callsign'=>(string)$atc['callsign']]);
    clearanceReply(true,['message'=>'clearance_saved','clearance'=>['departure_runway'=>$runway,'landing_runway'=>$landingRunway,'gate'=>$gate,'sid'=>$sid,'direct'=>$direct,'star'=>$star,'altitude'=>$altitude]]);
